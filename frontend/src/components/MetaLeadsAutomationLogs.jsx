...
const MetaLeadsAutomationLogs = () => {
    ...
    try {
      ...
      if (filterStatus === "failed") {
        const [{ total: failed }] = await Promise.all([
          getMetaLeadsAutomationLogsTotal({
            status: "failed",
            signal,
          }),
        ]);
  ...
  return (
    <div className="run-automation-logs" style={{ marginTop: '40px', borderTop: '1px solid #e2e8f0', paddingTop: '30px' }}>
      <div className="page-header">
        <div className="page-title">
          <h1>Meta Leads Automation Logs</h1>
              ...
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={r.id}>
                    ...
                    <td>{r?.metaLead?.name || "-"}</td>
                    <td>{r?.metaLead?.phone || "-"}</td>
                    <td>{r.stepIndex || "-"}</td>
...
export default MetaLeadsAutomationLogs;