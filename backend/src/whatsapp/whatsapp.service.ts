...
export class WhatsappService {
  ...
  async sendBulkTemplateMessageWithImage(contacts: Array<{name: string; phone: string}>, templateName: string, userId: number, settingsId?: number) {
    let settings;
    if (settingsId) {
      settings = await this.prisma.whatsAppSettings.findUnique({ where: { id: settingsId } });
      if (!settings) {
      ...
      try {
        ...
        await this.prisma.whatsAppMessage.create({
          data: {
            ...
            to: formattedPhone,
            from: formattedPhone,
            message: `Template ${templateName} with image sent to ${contact.name}`,
            direction: 'outgoing',
            status: 'sent',
            userId
          }
        ...
        const errorMsg = this.getErrorMessage(error);
        this.logger.error(`Failed to send to ${contact.phone}:`, errorMsg);
        results.push({ phoneNumber: formattedPhone, success: false, error: errorMsg });
      }
    }
